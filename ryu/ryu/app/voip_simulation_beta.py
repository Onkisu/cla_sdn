...
# ================= 4. TRAFFIC GENERATOR (SINE WAVE - FIXED) =================
def run_sine_wave_traffic(net):
    info(f"*** [Traffic] Starting SINE WAVE Pattern (Period: {WAVE_PERIOD}s)...\n")
    
    # 1. Start Receivers (Pastikan Log file dimatikan biar gak menuhin disk)
    for h in net.hosts:
        h.cmd("killall -9 ITGRecv")
        # -l /dev/null artinya jangan bikin log file di receiver
        h.popen(f"{ITG_RECV_BIN} -l /dev/null", shell=True)
    ...
    while not stop_event.is_set():
        ...
        # Hitung PPS (202 bytes per packet frame)
        wire_size = 202
        pps = int(target_bytes / wire_size)
        
        # Print status sesekali (setiap 10 detik) biar gak spam
        if int(step_time) % 10 == 0:
            phase = "UP" if sine_val > 0 else "DOWN"
            info(f"   >>> [SINE] {phase} | Target: {target_bytes} Bps ({pps} pps)\n")

        # B. EKSEKUSI (Gunakan POPEN agar tidak blocking)
        # Duration sedikit dilebihkan (2500ms) dibanding Interval (2000ms) untuk overlap
        duration_ms = int(UPDATE_SEC * 1000) + 500
        
        for h_name, meta in HOST_INFO.items():
            src_node = net.get(h_name)
            # -x /dev/null dan -l /dev/null penting agar buffer tidak penuh!
            cmd = (f"{ITG_SEND_BIN} -T UDP -a {meta['dst_ip']} "
                   f"-c 160 "
                   f"-C {pps} "
                   f"-t {duration_ms} "
                   f"-x /dev/null -l /dev/null") # Buang log receiver & sender
            
            # PENTING: Gunakan popen, bukan cmd
            src_node.popen(cmd, shell=True)
