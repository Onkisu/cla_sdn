...
def generate_client_traffic(client, server_ip, port, base_min_bw, base_max_bw, seed):
    ...
    while not stop_event.is_set():
        try:
            ...
            if "Connection refused" not in str(e) and "read failed" not in str(e):
...
def start_traffic(net):
    ...
    base_range_max = 5.0
    threads = [
        threading.Thread(target=generate_client_traffic, args=(h1, '10.0.1.1', 443, base_range_min, base_range_max, 12345), daemon=True), 
        threading.Thread(target=generate_client_traffic, args=(h2, '10.0.1.2', 443, base_range_min, base_range_max, 67890), daemon=True), 
        threading.Thread(target=generate_client_traffic, args=(h3, '10.0.2.1', 1935, base_range_min, base_range_max, 98765), daemon=True)
    ]
    for t in threads:
        t.start()
        # time.sleep(0.01) # Jeda kecil (opsional, mungkin nggak perlu lagi)
        
    return threads 

# ---------------------- [BARU] FUNGSI KOLEKTOR ----------------------
def get_host_interface_bytes(host):
    """Mendapatkan total RX bytes dari interface eth0 host."""
    try:
        # Jalankan 'ip -s link show eth0' di host
        # Outputnya kayak: RX: bytes packets errors dropped ...
        #                123456 123     0      0      ...
        cmd_output = host.cmd('ip -s link show eth0')
        
        # Cari baris RX bytes pake regex
        match = re.search(r'RX:.*?bytes\s+packets\s+errors.*?(\d+)\s+(\d+)\s+(\d+)', cmd_output, re.DOTALL)
        if match:
            rx_bytes = int(match.group(1))
            return rx_bytes
        else:
            info(f"  [Collector] Gagal parsing output 'ip -s link' untuk {host.name}\n")
            return None
    except Exception as e:
        info(f"  [Collector] Error get bytes for {host.name}: {e}\n")
        return None
...
def run_collector(net):
    ...
    info("\n*** Collector (via Host Interface) Dimulai ***\n")
    ...
    while not stop_event.is_set():
        ...
        for host in hosts_to_monitor:
            ...
            # 1. Ambil total byte saat ini dari interface host
            ...
            if delta_bytes > 0:
                ...
                pkts_rx = 0 # Kita nggak punya data packet dari 'ip link'
                
                info(f"  [Collector] Host: {host_name}, App: {app_name}, Delta Bytes: {delta_bytes}")
                
                rows_to_insert.append((
                    ts, 1, host.IP(), app_name, "udp", # dpid=1 (s1), host=IP host
                    host.IP(), "server_ip_dummy", host.MAC(), "mac_dummy", # src=host, dst=dummy
                    delta_bytes, delta_bytes, # tx/rx = delta
                    0, 0, # pkts = 0
                    latency, category
                 ))
...
if __name__ == "__main__":
    ...
    # [BARU] Mulai thread collector (yang sekarang jalan di dalem sini)
    collector_thread = threading.Thread(target=run_collector, args=(net,), daemon=True)
    ...
    try:
        # Kita join ke collector thread aja, biar nunggu dia selesai
        while collector_thread.is_alive():
            collector_thread.join(timeout=1.0) # Cek tiap detik
    ...
    # Tunggu collector thread mati (maksimal 2x interval)
    collector_thread.join(timeout=COLLECT_INTERVAL * 2) 
    
    # Tunggu traffic thread mati (kasih waktu dikit)
    # Kita nggak perlu join traffic thread karena mereka daemon
    time.sleep(1) 
