...
if __name__=="__main__":
    ...
    # init NAT node (default iptables inside namespace)
    nat.configDefault()
    nat.cmd("sysctl -w net.ipv4.ip_forward=1")

    # detect host uplink interface
    ...
    print("Host uplink interface:", uplink_if)

    # enable forwarding on host
    subprocess.call(["sysctl", "-w", "net.ipv4.ip_forward=1"])

    # host root iptables MASQUERADE
    subprocess.call(["iptables", "-t", "nat", "-C", "POSTROUTING", "-s", "10.0.0.0/8", "-o", uplink_if, "-j", "MASQUERADE"],
                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) or \
    subprocess.call(["iptables", "-t", "nat", "-A", "POSTROUTING", "-s", "10.0.0.0/8", "-o", uplink_if, "-j", "MASQUERADE"])
    ...
    # set DNS in all hosts
    for hname in ("h1","h2","h3","h4","h5","h6","h7"):
        h = net.get(hname)
        h.cmd("bash -c 'echo \"nameserver 8.8.8.8\" > /etc/resolv.conf'")

    print("NAT MASQUERADE & FORWARD rules applied. Hosts should reach internet now.")
