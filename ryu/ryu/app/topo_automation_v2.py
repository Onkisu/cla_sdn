...
class LinuxRouter(Node):
    """Router Linux dengan IP forwarding dan NAT"""
...
if __name__=="__main__":
    ...
    # deteksi interface uplink di host root
    ...
    # MASQUERADE di router r1 langsung
    r1.cmd("iptables -t nat -A POSTROUTING -o {} -j MASQUERADE".format(uplink_if))
    r1.cmd("iptables -A FORWARD -i r1-eth1 -o {} -j ACCEPT".format(uplink_if))
    r1.cmd("iptables -A FORWARD -i r1-eth2 -o {} -j ACCEPT".format(uplink_if))
    r1.cmd("iptables -A FORWARD -i r1-eth3 -o {} -j ACCEPT".format(uplink_if))
    r1.cmd("iptables -A FORWARD -i {} -o r1-eth1 -m state --state RELATED,ESTABLISHED -j ACCEPT".format(uplink_if))
    r1.cmd("iptables -A FORWARD -i {} -o r1-eth2 -m state --state RELATED,ESTABLISHED -j ACCEPT".format(uplink_if))
    r1.cmd("iptables -A FORWARD -i {} -o r1-eth3 -m state --state RELATED,ESTABLISHED -j ACCEPT".format(uplink_if))

    # enable IP forwarding di host root (penting!)
    ...
    # start forecast loop background
