...
from functools import partial # <-- [FIX 1] IMPORT INI
import threading
import random
import time
import subprocess
...
# (Semua fungsi helper SAMA PERSIS, tidak diubah)
    ...
    def __init__(self, spines=2, leafs=2, hosts_per_leaf=2):
        ...
        for l in range(1, leafs + 1):
            ...
            for s_idx, s_sw_name in enumerate(spine_switches):
                ...
                self.addLink(
                    leaf_sw, s_name, bw=100,
                    intfName1=leaf_intf_name, 
                    intfName2=spine_intf_name
                )
            ...
            for h in range(1, hosts_per_leaf + 1):
                ...
                self.addLink(
                    host_name, leaf_sw, bw=10, 
                    intfName1='eth0',
                    intfName2=switch_intf_name
                )
...
def generate_client_traffic(client, server_ip, port, base_min_bw, base_max_bw, seed):
    ...
    while not stop_event.is_set():
        try:
            ...
            burst_time = rng.uniform(0.5, 2.5); burst_time_str = f"{burst_time:.1f}"
...
def start_traffic(net):
    h1L1, h2L1 = net.get('h1L1', 'h2L1'); h1L2, h2L2 = net.get('h1L2', 'h2L2') 
...
if __name__ == "__main__":
    ...
    # --- [FIX 2 DI SINI] ---
    # Buat 'partial' class switch yang MENGUNCI protokol ke OpenFlow 1.3
    OVSSwitch13 = partial(OVSSwitch, protocols='OpenFlow13')
    # --- [SELESAI FIX 2] ---

    net = Mininet(topo=topo,
                  switch=OVSSwitch13, # <-- Gunakan class partial tadi
